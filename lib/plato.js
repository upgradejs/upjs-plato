...
async function inspect(files, outputDir, options, done) {
	...
	const dependencyReportsPromise = options.projectRoot
		? Promise.all([
			Promise.resolve(auditReport.process(options)),
			compatibilityReport.process(options),
			Promise.resolve(outdatedReport.process(options)),
		])
		: Promise.resolve();
	...
	if (!fileOutputDir) {
		...
		fs.mkdirp(fileOutputDir, async function() {
			await runReports(files, function() {
				...
				fs.copy(assets, path.join(outputDir, 'assets'), async function() {
					...
					// wait for dependency reports to finish before moving their output
					await dependencyReportsPromise;
					...
					if (options.projectRoot) {
						...
						['./yarn-audit.html', './npm-audit.html'].forEach(src => moveIfExists(src, dependenciesAudit));
						moveIfExists('./dependencies-compatibility.html', dependenciesCompatibility);
						['./yarn-outdated.html', './npm-outdated.html'].forEach(src => moveIfExists(src, dependenciesOutdated));
					}
					done(reports);
				});
			});
